...
def index_skupina(request, index, search_string = None):
        ...
        if request.session.get('perpage'):
            per_page = request.session['perpage']

        #paginacija
        paginator = Paginator(vsi_izdelki, per_page)
...
def pregled_narocil(request):
        ...
        if request.method == 'POST' and 'oddaj_narocilo' in request.POST:
                ...
                st_narocila = leto_dobavnice[-2:] + str(narocilo.id).zfill(4)
...
def natisni_narocilnica(request, narocilo):
        ...
        sidarta = '%s <br/>%s <br/>%s <br/> %s' % (podjetje_glavno.naslov, podjetje_glavno.tel_fax, podjetje_glavno.email, podjetje_glavno.spletna_stran)
...
def save_session(request):
    if request.method == 'POST':
        per_page = request.POST['perpage-select']
        request.session['perpage'] = int(per_page)

        return JsonResponse({'success':'Shranjeno'}, status=200)

    else:
        return JsonResponse({'error':'Request method is not POST'}, status=500)
